...
/**
 * Create a control port and register the Service as a BIP service on DNS-SD.
 * The service has a variable to give its state, the number of variable, the
 * number of inputs/outputs.
 * The control has the list of variables and the list of in/outputs for the service.
 * The description of variable and input are stored in object called VariableAttribut and
 * InOutputAttribut.
 * These data can be consulted by xml query on the control port.
 * In that purpose the class ControlClient enables to do these queries 
 * and stores the results.
 * <br>
 * The control server has three possibles status :
 * <ul><li>BEGIN : the service is not registered yet
 * </li><li>INIT : the service is registered, and can wait for data or other service
 * to begin processing
 * </li><li>RUNNING : the service is running : the service computes data. 
 * </li></ul>
 * <br>
 * Two methods needs to be reimplemented to manage correctly some queries.
 * That are connect and modifVariable. The first is called when an in/output is demanded
 * to connect a particular port, the second is called when there is a query for the 
 * modification of a variable.
 * 
 * Use:
 * <ul><li> Create a new instance of control server with the name of the service.
 * Eventually, you can implemented the methods connect and modifVariable if necessary.
 * </li><li> Register all the variable, and the in/outputs.
 * </li><li> Start the service (startServer). You can process query to the control server
 * in a processing loop (then call  processMessages()), or in another thread (then call 
 * startThreadProcessMessage()).
 * </li><li> Wait for data or other service. When you are ready, set the status value to RUNNING
 * and start processing
 * </li></ul>
 * 
 * An example of server is given in the method main. The service is called "essai".
 * It has two own variables : var_1 and var_2. var_1 can be modified by the user.
 * var_2 is regularly incremented. It has also one output that send regularly to all 
 * connected clients the message "hello".
 * 
