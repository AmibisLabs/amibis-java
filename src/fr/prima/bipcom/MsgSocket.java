...
/**
 * Manage the buffer to store bytes and parsed BIP messages
 * 
 * The buffer can receive byte. The method 'received' give the number of byte
 * added to the buffer. The method 'process' try to parse BIP message.
 * 
...
/**
 * Manage the message reception in a thread, when a message arrived the method
 * of the Bip Message listener is called. Bip message can be send by using the
 * method Send. Base of TcpServer and TcpClient
 * 
 * The byte reception, and detection of message is managed by a ReceiveBuffer
 * object.
 * 
 * @author Sebastien Pesnel
 */
