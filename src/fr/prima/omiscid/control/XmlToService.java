...
/**
 * Group methods to initialize service from an XML file.
 * The XML file contains the service description : 
 * the service name, the variable, the input, output and in_output 
 * Initialize a ControlServer object, add variables, add the in/outputs description.
 * 
...
public class XmlToService {
            ...
            if(node.getNodeType() == Node.ELEMENT_NODE){
                ...
                if(nodeName.equals("variable")){
                    createVariableFromXml(ctrlServer, (Element)node);
                         ...
                         nodeName.equals(InOutputAttribut.Output.getXMLTag()) ||
                         nodeName.equals(InOutputAttribut.InOutput.getXMLTag())){
    ...
    public static void createVariableFromXml(ControlServer ctrlServer, Element elt){
        ...
        VariableAttribut va  = ctrlServer.findVariable(varName);
        if(va == null) va = ctrlServer.addVariable(varName);
    ...
    public static void createIOFromXml(ControlServer ctrlServer, Element elt){
        ...
        InOutputKind iok = InOutputAttribut.IOKindFromName(elt.getNodeName());
        InOutputAttribut ioa = ctrlServer.findInOutput(ioName, iok);
        if(ioa == null) ioa = ctrlServer.addInOutput(ioName, null, iok);
