...
/**
 * Stores Variable Description. The variable description is composed of a value, a default value, a type a description, a description for the format, a kind of access  (Read, Read-Write, Read-Write before init). <br> The class gives methods to generate xml string with the description. Theses strings are used in exhange with the Control Server. <br> It also accepts several listeners interested in the variable modification. <br> Note: VariableAttribut do not contains the variable but only a description. The user must change the description when the variable change, for example when the value is modified. This coherence can be maintened with objects as IntVariableAttribut. IntVariableAttribut puts together the description for an int and the integer. It provides accessors that maintains the coherence between the description and the integer when the integer is modified.
 * @see fr.prima.omiscid.control.ControlServer
 * @see fr.prima.omiscid.control.IntVariableAttribut
...
public class VariableAttribut extends Attribut {
	 ...
	 * @uml.property  name="peerInterestedIn"
	 */
